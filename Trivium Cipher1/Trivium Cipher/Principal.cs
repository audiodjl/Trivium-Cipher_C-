    ...
    public partial class Principal : Form
    {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string HEXXA = TextoArchivo.Trim();
                ...
                var RRRR = ByteToBitArrayReverse(bytesFromHexa);

                //var ListaBytes = bytesFromHexa.Select(x => Convert.ToString(x, 2).PadLeft(8, '0')).ToArray();  //convertToBitList(new BitArray(bytesFromHexa));

                List<bool> listaFinal = new List<bool>();

                listaFinal = convertToBitList(RRRR);


                var ListaCifrada = Xor(listaFinal, keyStreamGen(listaFinal.Count, keyAndIVSetup(clave, VectorI)));
